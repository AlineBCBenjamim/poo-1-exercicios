...
app.post("/videos", async (req: Request, res: Response) => {
  try {
    ...
    const [videoDBExist]: TVideoDB[] | undefined[] = await db("videos").where({
      id,
    });
...
app.put("/videos/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [videoBD]: TVideoDB[] | undefined[] = await db("videos").where({
      id,
    });
    if (!videoBD) {
      res.status(400);
...
app.delete("/videos/:id", async (req: Request, res: Response) => {
  try {
    ...
    const [videoInDB]: TVideoDB[] | undefined[] = await db("videos").where({
      id,
    });
    ...
    res
      .status(200)
      .send({ message: "Video deletado com sucesso!", video: videoToDelete });
  } catch (error) {
    console.log(error);
